    ...
    public partial class GeneralDocumentsForm : MetroForm
    {
        ...
        public DataTable GeneralDocumentsTable(List<GeneralDocument> query)
        {
            ...
            foreach(var item in query)
            {
                ...
                newRow[0] = item.GeneralDocumentId;
                newRow[1] = item.GeneralDocumentName;
                newRow[2] = item.GeneralDoucmentFrom;
                newRow[3] = item.GeneralDocumentTo;
                newRow[4] = item.GeneralDocumentDateCreation.ToShortDateString();
        ...
        public async void metroTabControl1_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                using(ApplicationDbContext context = new ApplicationDbContext())
                {
                    ...
                    {
                        dataGridViewInput.DataSource = GeneralDocumentsTable(await documents.Where(d => d.DocumentType.DocumentTypeName == "Входящий").ToListAsync());
                    }
                    else if (metroTabControl1.SelectedIndex == 1)
                    {
                        dataGridViewOutput.DataSource = GeneralDocumentsTable(await documents.Where(d => d.DocumentType.DocumentTypeName == "Исходящий").ToListAsync());
                    }
                    else if (metroTabControl1.SelectedIndex == 2)
                    {
                        dataGridViewInside.DataSource = GeneralDocumentsTable(await documents.Where(d => d.DocumentType.DocumentTypeName == "Внутренний").ToListAsync());
                    }
                }
        ...
        {
            var Form = new GeneralDocumentsEntityForm();           
            Form.sdf = this;
            Form.Show();
        }
        ...
        private async void buttonInputDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dialog == DialogResult.Yes)
                {
                    using (ApplicationDbContext context = new ApplicationDbContext())
                    {
                        var d = await context.GeneralDocuments.FindAsync(dId);
        ...
        private void dataGridViewInput_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                using (ApplicationDbContext context = new ApplicationDbContext())
                {
                    richTextBoxDescription.Text = context.GeneralDocuments.Find(dId).GeneralDocumentDescription;
                }
            }
            catch (Exception gg)
            {
